...
try:
    ...
    element.screenshot(f"{var}_billing_name.png")
...
b1 = time.time()
try:
    element = wait.until(ec.visibility_of_element_located((By.ID, "billing_phone")))
    element.send_keys("9889798789")
    var1 = datetime.now().strftime("%d_%m_%Y_%H_%M_%S")
    driver.save_screenshot(f"{var1}_dummy_website.png")
except Exception as e:
    var1 = datetime.now().strftime("%d_%m_%Y_%H_%M_%S")
    driver.save_screenshot(f"{var1}_dummy_website.png")
    print(e)
