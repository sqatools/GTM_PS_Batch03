...
cart.click()
"""
items = driver.find_element(By.XPATH,"//div[@data-action='scroll']")
items= items.text
print(items)
list1 =list(items.split('\n'))
print(len(list1))
item_list=[]
price_list=[]
for i in range(0,len(list1),6):
    item_list.append(list1[i])
for j in range(2,len(list1),6):
    price_list.append(list1[j])
for x,y in zip(item_list,price_list):
    print(x,":",y)
